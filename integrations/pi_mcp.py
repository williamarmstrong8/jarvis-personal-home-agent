...
def live_instructions(*, wait_for_initial: bool = True) -> str:
    ...
    return (
        ...
        "To play on the Pi HDMI display, set play_movie on_display=true "
        "('on the tv', 'on the pi'). "
        "To pause, resume, stop, or change volume on that HDMI playback, "
        "call control_pi_display — do not curl Plex or pi_run_command. "
        "Confirmation cards and Spotify art are pushed "
        "to the Pi display automatically — do not also call show_card for those. "
        "Use pi_show_card for other confirmations. "
